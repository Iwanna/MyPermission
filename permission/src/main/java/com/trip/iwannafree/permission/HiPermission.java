...
//    private static HiPermission mInstance;
     ...
     * @param callback
     */
    public void checkSinglePermission(PermissionItem item, PermissionCallback callback) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            try {
                boolean bRet = checkPermission(mContext, item.Permission);
                if (bRet) {
                    return;
                }
            } catch (Exception e) {
     ...
     * @param callback
     */
    public void checkSpecialSinglePermission(final PermissionItem item, String setting, final PermissionCallback callback) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            try {
                boolean bRet = checkPermission(mContext, item.Permission);
                if (bRet) {
        ...
        mCallback = callback;
        mPermissionType = PermissionActivity.PERMISSION_TYPE_SPECIAL_SINGLE;
        mCheckPermissions = new ArrayList<>();
        mCheckPermissions.add(item);
        startActivity(setting);
    }

    private void startActivity(String setting) {
